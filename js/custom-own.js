  ...
  if (email != "") {
    if (email.indexOf("@", 0) < 0) {
      ...
      jQuery("#email_error").html(
        "Please enter your business email address. This form does not accept addresses from public/free domains."
      );
    } else {
      jQuery("#email_error").html(" ");
...
function verify_phonead(phone) {
  ...
  if (phone != "" && phone.length >= 10 && selval != "") {
    ...
    var pageid = jQuery("input#pageid").val();
    var baseURL =
      "/wp-content/themes/thegem-child/verifyPhone.php?id=" + pageid;
    ...
    jQuery.ajax({
      ...
      success: function (response) {
        ...
        if (res.status == false) {
          ...
          jQuery("#phone_error").html(
            '<span style="color:green">Valid Number</span>'
          );
          jQuery("#pver").val("1");
        }
      },
...
function verify_emailad1(email) {
  if (email != "") {
    if (email.indexOf("@", 0) < 0) {
      ...
      jQuery("#email_error1").html(
        "Please enter your business email address. This form does not accept addresses from yahoo.in."
      );
      return false;
    } else if (email.toLowerCase().slice(email.length - 10) == "rediff.com") {
      jQuery("#email_error1").html(
        "Please enter your business email address. This form does not accept addresses from rediff.com."
      );
    } else if (email.toLowerCase().slice(email.length - 11) == "outlook.com") {
      jQuery("#email_error1").html(
        "Please enter your business email address. This form does not accept addresses from outlook.com."
      );
    } else if (email.toLowerCase().slice(email.length - 11) == "hotmail.com") {
      jQuery("#email_error1").html(
        "Please enter your business email address. This form does not accept addresses from hotmail.com."
      );
    } else {
      jQuery("#email_error1").html(" ");
    }
...
jQuery(document).ready(function () {
  jQuery("p.btnRefresh").click(function () {
    ...
    jQuery("img.codeImg").attr(
      "src",
      "/wp-content/themes/thegem-child/cap2.php?id=" +
        p_id +
        "&p=img&rand=" +
        rand
    );
...
jQuery("#webtoLeadFormOne").click(function (e) {
  console.log("Home JS 6677 check cache");
  ...
  if (last_name == "" || email == "" || company == "" || vercode == "") {
    ...
    // var baseURLnew =  '/wp-content/themes/thegem-child/vwebtolead.php';
...
jQuery("#input-subs1").click(function (e) {
  console.log("this is custom own");
  e.preventDefault();
  var ids = ["emailfirst"];
  ...
  if (email == "") {
    ...
    var baseURL =
      "https://360smsapp.com/wp-content/themes/thegem-child/new-checkdata-footer.php";
    var form = jQuery("#formFirstLeadfooterHome");
    jQuery.ajax({
      url: baseURL,
      method: "post",
      data: form.serialize(),
